...
public sealed class Card3DView : MonoBehaviour
{
    ...
    [FormerlySerializedAs("monsterRoot")]
    [Tooltip("Parent for the monster actor prefab (from CardDefinition.monsterPrefab).")]
...
// e.g., outline or emissive toggle reference (optional)
    ...
    {
        if (WorldUIAnchor.TryGetComponent<MonsterWorldUI>(out var _worldUI))
            _worldUI.UpdateStats(
                BoundCard.Name,
                BoundCard.CurrentATK,
                BoundCard.CurrentDEF,
                BoundCard.IsFaceUp
            );
    }
}