...
public sealed class SpawnManager3D : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        if (_bus != null)
        {
            ...
            _bus.OnCardFaceChanged += HandleCardFaceChanged; // ⤵️ NEW
...
// SpawnManager3D.cs — add handler
    private void HandleCardFaceChanged(object sender, CardFaceChangedEvent e)
    {
        if (e?.Card == null) return;
        if (!_live.TryGetValue(e.Card, out var view) || !view) return;

        view.ApplyFace(e.IsFaceUp);

        // Manage monster actor visibility around flips
        if (e.Card.Def?.IsMonster == true)
        {
            if (e.IsFaceUp) view.EnsureMonsterSpawned();
            else            view.DespawnMonsterIfAny();
        }

        // Optional: refresh any world UI (ATK/DEF visible only when face-up)
        view.RefreshWorldUI();
    }
    private void HandleCardMoved(object sender, CardMovedEvent e)
    {
        ...
        // ✅ Only spawn monster actor if the card is a Monster AND is face-UP.
// Otherwise ensure the actor is despawned so only the flat card shows.
        if (to.Kind == BoardManager.CardZone.Monster && card.Def?.IsMonster == true)
        {
            if (card.IsFaceUp) v.EnsureMonsterSpawned();
            else               v.DespawnMonsterIfAny();
        }
        else
        {
            v.DespawnMonsterIfAny(); // ST/Field/etc: no monster actor
        }
    }
