...
namespace YGO.Duel.Battle
{
    ...
    public sealed class CardBattlerAdapter : IBattler
    {
        ...
        // CardBattlerAdapter.cs — inside class
        public void RevealForBattleIfNeeded()
        {
            if (_card == null || _card.IsFaceUp) return;

            // Preserve current battle position, only flip face-up
            var posWasAttack = _card.Position == CardBattlePosition.Attack;

            if (ServiceLocator.TryGet<PositionManager>(out var pm) && pm != null)
            {
                // If you have a dedicated "face" API, prefer it; otherwise reapply same pos with faceUp:true
                pm.RequestPositionChange(_card,
                    posWasAttack ? BattlePosition.Attack : BattlePosition.Defense,
                    faceUp: true, out _);
            }
            else
            {
                _card.SetPosition(posWasAttack ? CardBattlePosition.Attack : CardBattlePosition.Defense, faceUp: true);
            }

            if (ServiceLocator.TryGet<EventBus>(out var bus) && bus != null)
                bus.RaiseCardFaceChanged(_card, isFaceUp: true, reason: FaceChangeReason.BattleFlip);
        }
    }
}